...
%%
function [tau_p, tau_e, tau_b, l_p, l_e, l_b, rho_j, u_Ee, info] = get_tj_dacos(p, f)
  ...
  lT    = p(3);
  v_Hb  = p(4); % -, v_H^b = U_H^b g^2 kM^3/ (1 - kap) v^2; U_H^b = E_H^b/ {p_Am}: birth (embryo-larva transition)
  v_Hj  = p(5); % v_H^j = U_H^j g^2 kM^3/ (1 - kap) v^2; U_H^j = E_H^j/ {p_Am}
  v_Hp = p(6); % v_H^p = U_H^p g^2 kM^3/ (1 - kap) v^2; U_H^p = M_H^p/ {J_EAm} = E_H^p/ {p_Am}
  v_He  = p(7); % -, v_H^e = U_H^e g^2 kM^3/ (1 - kap) v^2; U_H^e = E_H^e/ {p_Am}: emergence (pupa-imago transition)
%   s_j   = p(5); % -, [E_R^j]/ [E_R^ref] scaled reprod buffer density at pupation
  ...
  kap   = p(8); % -, allocation fraction to soma of pupa
  kap_V = p(9); % -, conversion efficiency from larval reserve to larval structure, back to imago reserve
  
  if ~exist('f', 'var') || isempty(f)
    f = 1;
  end
  
  [tau_p, tau_j, tau_b, l_p, l_j, l_b, l_i, rho_j, rho_B, info] = get_tj(p, f);
